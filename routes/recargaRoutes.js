...
let historico = {}; // matriculas de exemplo para teste
...
router.get("/saldo/:matricula", (req, res) => {
  ...
  } else if (saldos[matricula] !== undefined) {
    res.json({
      matricula: matricula,
...
router.get('/historico/:matricula', (req, res) => {
  ...
  if (usuario && usuario.historico) {
    ...
    res.status(404).json({ erro: "Histórico não encontrado para esta matrícula." });
  }
});
...
router.post("/:matricula", (req, res) => {
  ...
  // atualiza o saldo com o valor da recarga
  if (!saldos[matricula]) {
    saldos[matricula] = 0;
  }
